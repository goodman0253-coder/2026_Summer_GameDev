					...
					if (enemys[i]->GetAlive() && bread->IsAlive())
					{
							...
							breadPos.y + breadSize.y > enemyPos.y - (enemySize.y / 2))
						{
						  ...
						  else if (donut != nullptr)
						  {
							  ...
							  // 繝峨・繝翫ヤ縺ｮ迴ｾ蝨ｨ縺ｮ繧ｵ繧､繧ｺ縺ｫ蠢懊§縺溘ム繝｡繝ｼ繧ｸ繧剃ｸ弱∴繧・
							  ...
							  // 繝峨・繝翫ヤ豸域ｻ・ｼ郁ｲｫ騾壹＆縺帙◆縺・ｴ蜷医・ Kill() 繧貞他縺ｰ縺ｪ縺・∈謚槭ｂ繧｢繝ｪ・・
							  donut->Kill();
						  }
						  else
						  {
...
void GameScene::Draw(void)
{
	...
	if (player != nullptr)
	{
		...
		// =========================================================
		// 霑ｽ蜉: 逕ｻ髱｢荳九↓隗｣謾ｾ貂医∩縺ｮ繝代Φ荳隕ｧ繧定｡ｨ遉ｺ・域ｨｪ荳ｦ縺ｳ・・
		// =========================================================
		int listStartX = 420;                // HP繧ｲ繝ｼ繧ｸ縺ｮ蜿ｳ蛛ｴ縺ｫ驟咲ｽｮ
		int listStartY = SCREEN_HEIGHT - 90; // 逕ｻ髱｢荳九・鬮倥＆
		int listIconSize = 64;               // 蜷・ヱ繝ｳ縺ｮ繧｢繧､繧ｳ繝ｳ繧ｵ繧､繧ｺ
		int spacing = 16;                    // 繧｢繧､繧ｳ繝ｳ蜷悟｣ｫ縺ｮ髢馴囈

		int maxTypes = static_cast<int>(Player::BREAD_TYPE::MAX);

		for (int i = 0; i < maxTypes; i++)
		{
			// 繝代Φ縺瑚ｧ｣謾ｾ縺輔ｌ縺ｦ縺・ｋ縺九メ繧ｧ繝・け
			bool isUnlocked = player->IsBreadUnlocked(i);

			// 蜷・ヱ繝ｳ縺ｮ逕ｻ蜒上ワ繝ｳ繝峨Ν繧帝∈謚・
			int iconHandle = -1;
			...
			if (isUnlocked)
			{
				// 迴ｾ蝨ｨ驕ｸ謚樔ｸｭ縺ｮ繝代Φ縺ｪ繧画棧繧帝ｻ・牡・医ワ繧､繝ｩ繧､繝茨ｼ峨√◎繧御ｻ･螟悶・逋ｽ縺ｧ陦ｨ遉ｺ[cite: 19]
			...
			{
				// 譛ｪ隗｣謾ｾ縺ｮ繝代Φ縺ｯ骰ｵ繝槭・繧ｯ鬚ｨ・郁埋縺・囓濶ｲ・九・縲阪・繝ｼ繧ｯ・峨〒陦ｨ迴ｾ
	...
	if (isClearTriggered)
	{
		// clearTimer(0?180)に応じて、アルファ値(0?255)を計算する
		int alpha = (int)(255.0f * ((float)clearTimer / 180.0f));
		if (alpha > 255) alpha = 255;

		// 画面を白で塗りつぶす（アルファ値で徐々に濃くする）
