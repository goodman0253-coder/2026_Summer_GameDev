...
{
#if 0
	enemys.push_back(new Enemy1, new Boss1()); // 敵のインスタンスを作成してリストに追加
		...
		for (int i = 0;i < enemys.size();i++)
		{
			if (enemys[i]->GetAlive() == true)
			{
				if (enemys[i] != nullptr)
				{
					...
					{
						if (PX + 23 < EX + (enemys[i]->GetEnemySize().x / 2) && // プレイヤーの右端が敵の左端より左にある場合
							PX + Player::PLAYER_WID - 23 > EX - (enemys[i]->GetEnemySize().x / 2) && // プレイヤーの左端が敵の右端より右にある場合
							PY < EY + (enemys[i]->GetEnemySize().y / 2) && // プレイヤーの下端が敵の上端より下にある場合
							PY + Player::PLAYER_HIG > EY - (enemys[i]->GetEnemySize().y / 2)) //プレイヤーの上端が敵の下端より上にある場合
						{
							player->ApplyDamage(); // プレイヤーにダメージを与える
